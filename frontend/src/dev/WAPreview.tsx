...
const channels = [
  ...
  { id: 2, name: "Satış Hattı", displayPhone: "+90 850 765 43 21", phoneNumberId: "222", wabaId: "998877", appId: "", graphVersion: "", hasToken: true, hasAppSecret: false, verifyToken: "t2", hookPath: "/api/v1/wa/hook/k2", surveyHookPath: "/api/v1/wa/survey/k2", active: true, settings: { ...settings, greeting: { ...settings.greeting, enabled: false } }, hasSurveySecret: false, verifiedName: "", qualityRating: "YELLOW", messagingLimit: "TIER_250", lastError: "Erişim anahtarının süresi dolmuş.", lastErrorAt: ago(40), memberIds: [1], createdAt: ago(300) },
];
const people = [
...
const contact = (id: number, name: string, wa: string) => ({ id, waId: wa, name: "", profileName: name, display: name, tags: id === 1 ? ["vip"] : [], note: "", optedOut: false, blocked: false });
...
const conversations = [
  { id: 1, channelId: 1, channelName: "Destek Hattı", contact: contact(1, "Zeynep Arslan", "905321112233"), ticket: ticket(1, { owner: { id: 1, name: "Toprak Şahin Güreli", hasAvatar: false }, participants: [{ id: 1, name: "Toprak Şahin Güreli", hasAvatar: false, role: "owner" }] }), last: { id: 5, direction: "in", kind: "text", preview: "Tamam teşekkürler, bekliyorum", at: ago(3), status: "received" }, unread: 1, teamReadId: 4, lastInboundAt: ago(3), windowEndsAt: new Date(Date.now() + 20 * 3600000).toISOString(), version: 10 },
...
const messages = [
  { id: 1, conversationId: 1, direction: "in", kind: "text", body: "Merhaba, internetim yarım saattir çalışmıyor.", status: "received", sender: { kind: "customer", name: "Zeynep Arslan" }, createdAt: ago(30) },
